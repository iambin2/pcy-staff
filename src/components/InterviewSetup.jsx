...
export default function InterviewSetup() {
  const [config, setConfig] = useState({ interval: 20, days: [] })
  const [forms, setForms] = useState({ applicantUrl: '', interviewerUrl: '' })
  ...
  useEffect(() => {
    getInterviewConfig()
      .then((r) => {
        if (r.ok) {
          setConfig(r.config?.days ? r.config : { interval: 20, days: [] })
  ...
  async function makeForms() {
    ...
    try {
      ...
      if (r.ok) {
        setForms({ applicantUrl: r.applicantUrl, interviewerUrl: r.interviewerUrl })
        setMsg('구글폼이 준비됐어요. 아래 링크를 공유하세요.')
      } else {
  ...
  return (
    <div className="setup">
      <div className="field field-inline">
        <span>면접 간격 (분)</span>
        <input className="num-sm" type="number" min={1} value={config.interval} onChange={(e) => setInterval(e.target.value)} />
      </div>
        ...
        <div className="setup-day" key={i}>
          ...
          {day.blocks.map((b, bi) => (
            <div className="setup-block" key={bi}>
              <input className="block-name" type="text" value={b.label} placeholder="이름(예: 오전)" onChange={(e) => setBlock(i, bi, 'label', e.target.value)} />
          ...
          <button type="button" className="cond-add" onClick={() => addBlock(i)}>+ 시간대 추가</button>
        </div>
      ))}
      ...
      <div className="setup-summary">
        ...
        {labels.length > 0 && <span className="setup-labels"> · 폼 선택지: {labels.join(' / ')}</span>}
      </div>
      ...
      <div className="setup-actions">
        ...
        <button type="button" className="btn" onClick={makeForms} disabled={creating || totalSlots === 0}>
          {creating ? '폼 만드는 중…' : '구글폼 만들기 / 갱신'}
        </button>
      </div>

      {(forms.applicantUrl || forms.interviewerUrl) && (
        <div className="setup-forms">
          <div className="form-link">
            <span>지원자용 폼 (지원자들에게 공유)</span>
            <a href={forms.applicantUrl} target="_blank" rel="noreferrer">{forms.applicantUrl}</a>
          </div>
          <div className="form-link">
